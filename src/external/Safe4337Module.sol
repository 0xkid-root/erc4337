    ...
    Initializable
{
     ...
     */
    address public SUPPORTED_ENTRYPOINT;

    address public constant HYPT_FACTORY = 0x0000000000000000000000000000000000000888;

    function initialize_Safe4337(address entryPoint) internal {
     ...
     */
    function _checkSignaturesLength(
        ...
        for (uint256 i = 0; i < threshold; i++) {
            ...
            if (signatureType == 0) {
                uint256 signatureOffset = uint256(
                    bytes32(signatures[signaturePos + 0x20:])
                );
                uint256 signatureLength = uint256(
                    bytes32(signatures[signatureOffset:])
                );
    ...
    function _validateSignatures(
        ...
        bool validSignature = _checkSignaturesLength(
            signatures,
